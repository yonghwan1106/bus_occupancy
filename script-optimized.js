...
(function() {
    ...
    function loadSVGDiagrams() {
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // 해당 탭의 SVG 로드
                    const tabId = entry.target.id.replace('-content', '');
                    loadSVG(tabId);
                    // 한 번 로드 후 관찰 중단
        ...
        // 첫 번째 탭의 SVG는 즉시 로드 (기본적으로 표시되는 탭)
        loadSVG('system');
    }
    ...
    function loadSVG(tabId) {
        ...
        fetch(`images/${tabId}.svg`)
            ...
            .catch(error => {
                console.warn(`SVG 로드 실패 (${tabId}):`, error);
                // 오류 시 기본 SVG 사용 (이미 HTML 메인 파일에 있는)
            });
    }
